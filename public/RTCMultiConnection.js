(function() {
    ...
    window.RTCMultiConnection = function(channel) {
        ...
        connection.send = function(data, _channel) {
            ...
            if (!isNull(data.size) && !isNull(data.type)) {
                ...
                rtcMultiSession.fileBufferReader.readAsArrayBuffer(
                    data,
                    function(uuid) {
                        rtcMultiSession.fileBufferReader.getNextChunk(
                            uuid,
                            function(nextChunk, isLastChunk, extra) {
        ...
        function joinSession(session, joinAs) {
            ...
            if (!rtcMultiSession) {
                ...
                initRTCMultiSession(function() {
                    log(
                        "Signaling channel is connected. Joining the session again..."
                    );
                    setTimeout(function() {
            ...
            if (!session || !session.userid || !session.sessionid) {
                ...
                var error =
                    'Invalid data passed over "connection.join" method.';
        ...
        function captureUserMedia(callback, _session, dontCheckChromExtension) {
            ...
            if (
                isData(session) ||
                (!connection.isInitiator && session.oneway)
            ) {
            ...
            function _captureUserMedia(
                ...
                if (
                    ...
                    if (!navigator.customGetUserMediaBar) {
                        loadScript(
                            connection.resources.customGetUserMediaBar,
                            function() {
                                _captureUserMedia(
                                    forcedConstraints,
                                    forcedCallback,
                            ...
                            if (isChrome && (session.audio || session.video)) {
                                ...
                                DetectRTC.load(function() {
                                    ...
                                    if (
                                        ...
                                        !connection.getUserMediaPromptedOnce
                                    ) {
        ...
        function onStreamSuccessCallback(
            ...
            connection.onstatechange({
                ...
                reason:
                    "Captured user media using constraints: " +
                    toStr(forcedConstraints)
            });
            ...
            connection.streams[streamid] = connection._getStream(
                streamedObject
            );
        ...
        connection.removeStream = function(streamid, dontRenegotiate) {
            ...
            if (
                ...
                function _detachStream(_stream, config) {
                    ...
                    if (
                        connection.detachStreams.indexOf(_stream.streamid) != -1
                    ) {
    ...
    function RTCMultiSession(connection, callbackForSignalingReady) {
        ...
        function onNewSession(session) {
            ...
            if (connection.onNewSession) {
                session.join = function(forceSession) {
                    ...
                    var isDontCaptureUserMedia =
                        connection.dontCaptureUserMedia;
        ...
        function newPrivateSocket(_config) {
            ...
            var peerConfig = {
                ...
                onicecandidate: function(candidate) {
                    ...
                    if (
                        !turn &&
                        !!candidate.candidate.match(/a=candidate.*typ relay/g)
                    )
                        return;
                    if (
                        !stun &&
                        !!candidate.candidate.match(/a=candidate.*typ srflx/g)
                    )
                        return;
                    ...
                    if (abToStr.indexOf('"userid":') != -1) {
                        ...
                        fileBufferReader.convertToObject(data, function(chunk) {
                            if (chunk.maxChunks || chunk.readyForNextChunk) {
                                ...
                                fileBufferReader.addChunk(chunk, function(
                                    promptNextChunk
                                ) {
                                    // request next chunk
                ...
                onaddstream: function(stream, session) {
                    session =
                        session || _config.renegotiate || connection.session;
                    ...
                    if (_config.streaminfo) {
                        ...
                        var strInfo = JSON.parse(
                            streaminfo[streaminfo.length - 1]
                        );
                    ...
                    if (!isPluginRTC && !stream.getVideoTracks().length) {
                        ...
                        return mediaElement.addEventListener(
                            "play",
                            eventListener,
                            false
                        );
                    ...
                    if (connection.peers[_config.userid]) {
                        ...
                        if (
                            ...
                            warn(
                                ...
                                toStr(
                                    connection.peers[_config.userid].peer
                                        .connection
                                ),
                                "ReDialing.."
                            );
            ...
            function onChannelOpened(channel) {
                ...
                if (
                    ...
                    connection.partOfScreen.sharing
                ) {
            ...
            function updateSocket() {
                ...
                connection.peers[_config.userid] = {
                    ...
                    removeStream: function(streamid) {
                        if (!connection.streams[streamid])
                            return warn(
                                "No such stream exists. Stream-id:",
                                streamid
                            );
                    ...
                    getConnectionStats: function(callback, interval) {
                        ...
                        if (!window.getConnectionStats) {
                            loadScript(
                                connection.resources.getConnectionStats,
                                invoker
                            );
                        } else invoker();
                        ...
                        function invoker() {
                            ...
                            peer.connection &&
                                peer.connection.getConnectionStats(
                                    callback,
                                    interval
                                );
                        }
            ...
            function onSessionOpened() {
                ...
                if (
                    _config.userinfo.browser == "chrome" &&
                    !_config.renegotiatedOnce
                ) {
            ...
            function socketResponse(response) {
                ...
                if (response.streamid) {
                    ...
                    if (!rtcMultiSession.streamids[response.streamid]) {
                        rtcMultiSession.streamids[response.streamid] =
                            response.streamid;
                    ...
                    if (response.promptMuteUnmute) {
                        ...
                        if (connection.streams[response.streamid]) {
                            if (
                                ...
                                connection.streams[response.streamid].mute(
                                    response.session
                                );
                            }
                            if (
                                ...
                                connection.streams[response.streamid].unmute(
                                    response.session
                                );
                            }
                        }
                        ...
                        fakeObject.isAudio =
                            !!fakeObject.session.audio &&
                            !fakeObject.session.video;
                        fakeObject.isVideo = !!fakeObject.session.video;
                ...
                if (
                    response.promptStreamStop /* && !connection.isInitiator */
                ) {
                ...
                if (response.playRoleOfBroadcaster) {
                    ...
                    if (response.participants) {
                        ...
                        if (
                            sockets[0] &&
                            sockets[0].userid == response.userid
                        ) {
                            delete sockets[0];
                ...
                if (response.changeBandwidth) {
                    ...
                    connection.peers[response.userid].peer.bandwidth =
                        response.bandwidth;
                ...
                if (response.customMessage) {
                    ...
                    if (response.message.ejected) {
                        if (
                            connection.sessionDescriptions[connection.sessionid]
                                .userid != response.userid
                        ) {
                        ...
                        connection.peers[response.userid].onCustomMessage(
                            response.message
                        );
                }
                ...
                if (response.hold || response.unhold) {
                    ...
                    connection.peers[
                        response.userid
                    ].peer.hold = !!response.hold;
                    connection.peers[response.userid].peer.holdMLine =
                        response.holdMLine;
            ...
            function sdpInvoker(sdp, labels) {
                ...
                function createAnswer() {
                    peer.recreateAnswer(sdp, session, function(
                        _sdp,
                        streaminfo
                    ) {
                        sendsdp({
        ...
        function onNewParticipant(response) {
            ...
            newPrivateSocket({
                channel: new_channel,
                extra: response.userData
                    ? response.userData.extra
                    : response.extra,
        ...
        connection.remove = function(userid) {
            if (
                ...
                rtcMultiSession.requestsFrom[userid]
            )
            ...
            return connection.openSignalingChannel({
                onmessage: function(response) {
                    ...
                    if (response.sessionid && response.userid) {
                        if (
                            !connection.sessionDescriptions[response.sessionid]
                        ) {
                            connection.numberOfSessions++;
                            connection.sessionDescriptions[
                                response.sessionid
                    ...
                    if (response.acceptedRequestOf == connection.userid) {
                        connection.onstatechange({
                            ...
                            reason:
                                response.userid +
                                " accepted your participation request."
                        });
                    }
                    ...
                    if (response.rejectedRequestOf == connection.userid) {
                        connection.onstatechange({
                            ...
                            reason:
                                response.userid +
                                " rejected your participation request."
                        });
                    }
                    ...
                    if (
                        ...
                        if (response.presenceState == "available") {
                            ...
                            connection.onstatechange({
                                ...
                                name: "room-available",
                                reason:
                                    "Initiator is available and room is active."
                            });
        ...
        this.send = function(message, _channel) {
            if (
                !(message instanceof ArrayBuffer || message instanceof DataView)
            ) {
                message = str2ab({
                    extra: connection.extra,
                    userid: connection.userid,
    ...
    function createMediaElement(stream, session) {
        ...
        mediaElement[isFirefox ? "mozSrcObject" : "src"] = isFirefox
            ? stream
    ...
    function getUserMedia(options) {
        ...
        if (videoMandatoryConstraints) {
            ...
            if (videoMandatoryConstraints.minAspectRatio) {
                mandatory.minAspectRatio =
                    videoMandatoryConstraints.minAspectRatio;
            }
            ...
            if (mandatory.minWidth && mandatory.minHeight) {
                ...
                if (
                    allowed.indexOf(
                        mandatory.minWidth + ":" + mandatory.minHeight
                    ) == -1 ||
                    allowed.indexOf(
                        mandatory.maxWidth + ":" + mandatory.maxHeight
                    ) == -1
                ) {
                    error(
    ...
    if (typeof mozRTCPeerConnection !== "undefined") {
        ...
        console.error(
            "WebRTC 1.0 (RTCPeerConnection) API seems NOT available in this browser."
        );
    }
        ...
        return {
            ...
            init: function() {
                ...
                this.connection.onaddstream = function(e) {
                    log(
                        "onaddstream",
                        isPluginRTC ? e.stream : toStr(e.stream)
                    );
                ...
                this.connection.onsignalingstatechange = function() {
                    self.connection &&
                        self.oniceconnectionstatechange({
                            iceConnectionState:
                                self.connection.iceConnectionState,
                            iceGatheringState:
                                self.connection.iceGatheringState,
            ...
            setConstraints: function() {
                var sdpConstraints = setSdpConstraints({
                    ...
                    OfferToReceiveVideo:
                        !!this.session.video || !!this.session.screen
                });

                if (this.sdpConstraints.mandatory) {
                    sdpConstraints = setSdpConstraints(
            ...
            setRemoteDescription: function(sessionDescription, onSdpSuccess) {
                ...
                this.connection.setRemoteDescription(
                    ...
                    function(error) {
                        if (
                            error.search(
                                /STATE_SENTINITIATE|STATE_INPROGRESS/gi
                            ) == -1
                        ) {
            ...
            setChannelEvents: function(channel) {
                ...
                channel.onopen = function() {
                    ...
                    channel.send = function(data) {
                        if (
                            self.connection.iceConnectionState == "disconnected"
                        ) {
                            return;
                        }

                        if (
                            channel.readyState.search(/closing|closed/g) != -1
                        ) {
                            return;
                        }

                        if (
                            channel.readyState.search(/connecting|open/g) == -1
                        ) {
                            return;
                        }
        ...
        */
        ...
        if (!!navigator.mozGetUserMedia) {
            hyperlink.onclick = function() {
                (document.body || document.documentElement).removeChild(
                    hyperlink
                );
            };
    ...
    (function() {
        ...
        function getBrowserInfo() {
            ...
            if (isEdge) {
                ...
                fullVersion = parseInt(
                    navigator.userAgent.match(/Edge\/(\d+).(\d+)$/)[2],
                    10
                );
            }
    ...
    function setDefaults(connection) {
        ...
        connection.chunkSize =
            isFirefox || chromeVersion >= 32 ? 13 * 1000 : 1000; // 1000 chars for RTP and 13000 chars for SCTP
        ...
        connection.onstream = function(e) {
            connection.body.insertBefore(
                e.mediaElement,
                connection.body.firstChild
            );
        };
        ...
        connection._getStream = function(event) {
            ...
            resultingObject.startRecording = function(session) {
                ...
                if (isFirefox) {
                    ...
                    if (
                        self.stream.getAudioTracks().length &&
                        self.stream.getVideoTracks().length
                    ) {
                    ...
                    if (
                        ...
                        connection.DetectRTC.browser.version >= 50 &&
                        self.stream.getAudioTracks().length &&
                        self.stream.getVideoTracks().length
                    ) {
                        self.videoRecorder = RecordRTC(self.stream, {
        ...
        connection.drop = function(config) {
            ...
            connection.sendCustomMessage({
                drop: true,
                dontRenegotiate: isNull(config.renegotiate)
                    ? true
                    : config.renegotiate
            });
        };
        ...
        connection.shareMediaFile = function(file, video, streamerid) {
            ...
            if (!PreRecordedMediaStreamer) {
                loadScript(
                    connection.resources.PreRecordedMediaStreamer,
                    function() {
        ...
        connection.onconnected = function(event) {
            ...
            log(
                "Peer connection has been established between you and",
                event.userid
            );
        };
        ...
        connection.openSignalingChannel = function(config) {
            ...
            var firebase = new Firebase(
                connection.resources.firebaseio + channel
            );
